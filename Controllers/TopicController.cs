...
                    // กันซ้ำ (ถ้าคุณอยากให้ซ้ำได้ ลบส่วนนี้ได้)
                    var exists = await _context.Topics.AnyAsync(t => t.TopicTitle == title);
                    if (exists)
                        return BadRequest(new { Message = "มี Topic นี้อยู่แล้ว" });
...
            [HttpDelete("{id:int}")]
            public async Task<IActionResult> Delete(int id)
            {
                ...
                {
                    // ถ้า Topic ถูกใช้เป็น FK (เช่น inbox.title_id) จะลบไม่ได้ → จะตกมาที่นี่
                    return BadRequest(new
                    {
                        Message = "ลบข้อมูลไม่สำเร็จ (อาจมีข้อมูลอื่นใช้งาน Topic นี้อยู่)",
                        Error = ex.InnerException?.Message ?? ex.Message
                    });
                }
                catch (Exception ex)
